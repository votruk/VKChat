		...
		if (mMessage.getAttachments() != null && mMessage.getAttachments().size() != 0) {
			for (Attachment attachment : mMessage.getAttachments()) {
				if (attachment.getType().equals("photo")) {
					view.findViewById(R.id.message_layout_photos_left).setVisibility(View.VISIBLE);
					...
					((LinearLayout)view.findViewById(R.id.message_layout_photos_left)).addView(draweeView);
				}
			}
		}
	}
